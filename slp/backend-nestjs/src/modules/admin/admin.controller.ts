...
export class AdminController {
  ...
  // Quizzes
  @Get('quizzes')
  async getQuizzes(): Promise<QuizAdminDto[]> {
  ...
  // Comments
  @Get('comments')
  async getComments(@Query('includeDeleted') includeDeleted: string = 'false'): Promise<CommentAdminDto[]> {
    return this.adminService.getAllComments(includeDeleted === 'true');
  }
  ...
  // Logs
  @Get('logs')
  async getLogs(@Query('count') count: string = '100'): Promise<AdminLogDto[]> {
    return this.adminService.getRecentLogs(parseInt(count, 10));
  }
}