...
export class AuthController {
    ...
    if (!result.success) {
      switch (result.errorCode) {
        ...
        case 'INVALID_PASSWORD':
          throw new UnauthorizedException({ code: result.errorCode, message: result.message });
        case 'ACCOUNT_BANNED':
          throw new UnauthorizedException({ code: result.errorCode, message: result.message });
        case 'EMAIL_NOT_VERIFIED':
          throw new UnauthorizedException({ code: result.errorCode, message: result.message });
        default:
          throw new UnauthorizedException({ code: 'LOGIN_FAILED', message: 'Login failed' });
      }
    }
    return result.data;
  }
  ...
  async resetPassword(@Body() body: ResetPasswordRequest) {
    const success = await this.authService.confirmPasswordReset(body.token, body.newPassword);
    if (!success) {
  ...
  async verifyEmail(@Body() body: VerifyEmailRequest) {
    ...
    if (!success) {
      throw new UnauthorizedException({ message: 'Invalid verification token' });
    }
