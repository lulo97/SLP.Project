...
<template>
  <a-modal
    ...
    data-testid="comment-history-modal"
  >
    <div v-if="store.historyLoading" class="text-center py-6">
      <a-spin />
    </div>

    <div v-else-if="store.history.length === 0" class="text-gray-400 text-sm py-4">
    ...
    <a-timeline v-else>
      ...
      >
        <div class="flex items-center justify-between mb-1">
          ...
          <a-tag v-if="index === store.history.length - 1" color="blue" class="text-xs">
            Latest saved
          </a-tag>
        </div>
