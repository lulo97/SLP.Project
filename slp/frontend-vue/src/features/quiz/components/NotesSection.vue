<template>
  <a-card title="My Notes" class="shadow-sm">
    ...
    <div v-else-if="notes.length === 0" class="text-gray-400 text-sm py-2" data-testid="no-notes-message">
      No notes yet.
    </div>
    <div v-else class="space-y-3">
      ...
      >
        <div class="flex justify-between items-start mb-2">
          ...
          <a-button @click="emit('remove', note.id)" type="text" danger size="small" :data-testid="`delete-note-${note.id}`">
            <DeleteOutlined />
          </a-button>
        </div>
    ...
    <a-button @click="openAddModal" block type="dashed" class="mt-2" data-testid="add-note-button">
    ...
    >
      <a-form :model="form" layout="vertical">
        <a-form-item label="Title" required>
          <a-input v-model:value="form.title" data-testid="note-title-input" />
        </a-form-item>
        <a-form-item label="Content" required>
          <a-textarea v-model:value="form.content" :rows="4" data-testid="note-content-input" />
        </a-form-item>
      </a-form>
    </a-modal>
  </a-card>
