<template>
  ...
  <MobileLayout :title="source?.title || 'Source'">
    ...
    >
      ...
      >
        ...
        >
          <p v-for="(para, i) in paragraphs" :key="i" class="mb-[1.4em]">
            {{ para }}
          </p>
        </div>
  ...
  <Transition name="notif">
    <div
      v-if="notification"
      class="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 px-[18px] py-2.5 rounded-[24px] text-[13px] font-medium z-[9000] whitespace-nowrap shadow-[0_4px_20px_rgba(0,0,0,0.15)]"
      :class="{
...
async function loadExplanations() {
  ...
  try {
    // FIX: was "/sources/${sourceId.value}/explanations" (plural) which is wrong.
    // The backend controller is mounted at /api/source (singular).
    const res = await apiClient.get<ExplanationItem[]>(
      `/source/${sourceId.value}/explanations`,
    );
    explanations.value = res.data;
  } catch {
    /* no explanations yet — fail silently */
  } finally {
...
async function submitExplanation(text: string) {
  ...
  try {
    ...
    if (idx !== -1) explanations.value[idx] = saved.data;
  } catch (err: any) {
    const idx = explanations.value.findIndex((e) => e.id === placeholderId);
...
function handleTts(selectedText: string) {
  window.getSelection()?.removeAllRanges();
  ttsPlay(selectedText);
}
...
// ── Notifications ─────────────────────────────────────────────────────────────
function showNotif(msg: string, type: string, icon: any) {
  notification.value = { message: msg, type, icon };
