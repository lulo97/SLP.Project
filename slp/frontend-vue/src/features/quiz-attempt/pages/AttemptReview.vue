  ...
  <MobileLayout :title="`Review: ${review?.quizTitle || 'Attempt'}`">
    <div v-if="review" class="space-y-4 pb-6" data-testid="review-container">
      ...
      <a-card class="shadow-sm" data-testid="score-card">
        <div class="text-center py-2">
          ...
          <div class="flex items-center justify-center gap-4 text-sm text-gray-500 mb-3">
            ...
            <span data-testid="incorrect-count">{{ incorrectCount }} incorrect</span>
            <span class="text-gray-300">|</span>
            <span data-testid="completed-time">
              Completed {{ new Date(review.endTime!).toLocaleString() }}
            </span>
          </div>
          <div class="h-2 bg-gray-100 rounded-full overflow-hidden" data-testid="score-progress-track">
            <div
      ...
      >
        ...
        <div class="flex items-start gap-2 mb-3">
          ...
          <div class="flex-1">
            <div class="flex items-center gap-2 mb-1">
              ...
              >
                {{ isFlashcard(ans) ? '✦ Informational' : ans.isCorrect ? '✓ Correct' : '✗ Incorrect' }}
              </span>
              <span
        ...
        >
          <p class="text-xs font-semibold text-yellow-700 mb-1.5">Flashcard content</p>
...
const correctCount = computed(() =>
  review.value?.answerReview?.filter((a: any) => !isFlashcard(a) && a.isCorrect).length ?? 0,
);
  ...
  setup(props) {
    return () => {
      ...
      return h('div', { class: 'flex flex-col gap-1' }, [
        h('span', { class: 'text-sm', 'data-testid': 'flashcard-review-front' },
          `Front: ${meta.front ?? '—'}`),
        h('span', { class: 'text-sm text-gray-500', 'data-testid': 'flashcard-review-back' },
          `Back: ${meta.back ?? '—'}`),
      ]);
    };
  },
...
const AnswerDisplay = defineComponent({
  ...
  setup(props) {
    return () => {
      ...
      const noAnswer = () =>
        h('span', { class: 'text-xs text-gray-400 italic', 'data-testid': 'answer-display-empty' }, 'No answer');
      ...
      if (type === 'multiple_choice') {
        ...
        return h('div', { class: 'flex flex-col gap-1', 'data-testid': 'answer-display-multiple-choice' },
          ids.map((id: any) => {
            const opt = options.find((o: any) => String(o.id) === String(id));
            return h('span', { class: 'text-sm', 'data-testid': `answer-display-option-${id}` }, `• ${opt?.text ?? id}`);
          }),
        );
      }
      ...
      if (type === 'single_choice') {
        ...
        const opt = options.find((o: any) => String(o.id) === String(id));
        return h('span', { class: 'text-sm', 'data-testid': 'answer-display-single-choice' }, opt?.text ?? String(id));
      }
      ...
      if (type === 'true_false') {
        if (props.userAnswer) {
          ...
          if (raw === null || raw === undefined || typeof raw !== 'boolean')
            return h('span', { class: 'text-xs text-gray-400 italic', 'data-testid': 'answer-display-empty' }, 'No answer');
          return h('span', { class: 'text-sm font-medium', 'data-testid': 'answer-display-true-false' }, raw ? 'True' : 'False');
        }
        ...
        return h('span', { class: 'text-sm font-medium', 'data-testid': 'answer-display-true-false' }, correctVal ? 'True' : 'False');
      }
      ...
      if (type === 'fill_blank') {
        if (props.userAnswer) {
          ...
          return h('span', { class: 'text-sm font-medium', 'data-testid': 'answer-display-fill-blank' }, answer);
        }
        ...
        return h('span', { class: 'text-sm font-medium', 'data-testid': 'answer-display-fill-blank' }, answers.join(' / '));
      }
      ...
      if (type === 'ordering') {
        ...
        if (props.userAnswer) {
          ...
          return h('div', { class: 'flex flex-col gap-1', 'data-testid': 'answer-display-ordering' },
            order.map((id: number, i: number) => {
              const item = items.find((it: any) => it.order_id === id);
              return h('span', { class: 'text-sm', 'data-testid': `answer-display-ordering-item-${i}` }, `${i + 1}. ${item?.text ?? id}`);
            }),
          );
        }
        ...
        return h('div', { class: 'flex flex-col gap-1', 'data-testid': 'answer-display-ordering' },
          correctItems.map((item: any, i: number) =>
            h('span', { class: 'text-sm', 'data-testid': `answer-display-ordering-item-${i}` }, `${i + 1}. ${item.text}`),
          ),
        );
      }
      ...
      if (type === 'matching') {
        ...
        if (props.userAnswer) {
          ...
          return h('div', { class: 'flex flex-col gap-1', 'data-testid': 'answer-display-matching' },
            matches.map((m: any) => {
        ...
        return h('div', { class: 'flex flex-col gap-1', 'data-testid': 'answer-display-matching' },
          pairs.map((p: any) =>
            h('span', { class: 'text-sm text-green-700', 'data-testid': `answer-display-pair-${p.id}` }, `${p.left} → ${p.right}`),
          ),
        );
      }

      return h('span', { class: 'text-xs text-gray-400 italic', 'data-testid': 'answer-display-na' }, 'N/A');
    };
  },
});
</script>