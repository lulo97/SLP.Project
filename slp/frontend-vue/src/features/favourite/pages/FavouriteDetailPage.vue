<template>
  <MobileLayout :title="favorite?.text || t('favourite.favouriteDetail')">
    ...
    <div v-else-if="favorite" class="space-y-4">
      <div class="bg-white rounded-lg p-4 shadow-sm">
        <div class="flex items-center gap-2 mb-2">
          <h1 class="text-2xl font-semibold">{{ favorite.text }}</h1>
          <a-tag :color="getTypeColor(favorite.type)">{{ getTypeLabel(favorite.type) }}</a-tag>
        </div>
        ...
        <div v-if="favorite.note" class="mt-4">
          <h3 class="font-semibold mb-2">{{ t('favourite.note') }}:</h3>
          <p class="whitespace-pre-wrap">{{ favorite.note }}</p>
        </div>
      </div>
