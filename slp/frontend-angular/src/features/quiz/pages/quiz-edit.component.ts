// src/features/quiz/pages/quiz-edit.component.ts
...
@Component({
  ...
  template: `
    <div *ngIf="loading" class="text-center py-8" data-testid="quiz-edit-loading">
      <nz-spin />
    </div>
...
export class QuizEditComponent implements OnInit, OnDestroy {
  ...
  openQuestionModal(action: 'add' | 'edit' | 'insert', question?: DisplayQuestion, index?: number): void {
  ...
  handleSelectQuestion(selectedQuestion: any): void {
    const snapshot = {
      ...
      metadata: selectedQuestion.metadataJson ? JSON.parse(selectedQuestion.metadataJson) : {},
      tags: selectedQuestion.tags || [],
    };
    const snapshotJson = JSON.stringify(snapshot);
    const newOrder = this.questions.length ? Math.max(...this.questions.map(q => q.displayOrder)) + 1 : 1;
    this.quizService.createQuizQuestion(this.quizId, snapshotJson, newOrder, selectedQuestion.id).subscribe({
      next: (created) => {
  ...
  handleQuestionSaved(payload: CreateQuestionPayload): void {
    ...
    if (this.editingQuestion) {
      // Update existing
      this.quizService.updateQuizQuestion(this.editingQuestion.id, snapshotJson, this.editingQuestion.displayOrder).subscribe({
        next: () => {
          const idx = this.questions.findIndex(q => q.id === this.editingQuestion!.id);
          if (idx !== -1) {
            this.questions[idx] = { ...this.questions[idx], ...snapshot, questionSnapshotJson: snapshotJson };
            this.questions = [...this.questions];
          }
          this.message.success(this.translate.instant('quiz.questionUpdated'));
          this.editingQuestion = undefined;
        },
        error: () => this.message.error(this.translate.instant('quiz.updateQuestionError')),
      });
    } else {
      // Create new
      let newOrder: number;
      if (this.insertIndex !== undefined) {
        ...
        const toShift = this.questions.filter(q => q.displayOrder >= newOrder);
        Promise.all(toShift.map(q => this.quizService.updateQuizQuestion(q.id, q.questionSnapshotJson, q.displayOrder + 1).toPromise())).then(() => {
          ...
          return this.quizService.createQuizQuestion(this.quizId, snapshotJson, newOrder).toPromise();
        }).then(created => {
          if (created) {
            const newQuestion: DisplayQuestion = {
              id: created.id,
            ...
            this.questions = [...this.questions, newQuestion].sort((a, b) => a.displayOrder - b.displayOrder);
            this.message.success(this.translate.instant('quiz.questionCreated'));
          }
        }).catch(() => this.message.error(this.translate.instant('quiz.createQuestionError')));
      } else {
        newOrder = this.questions.length ? Math.max(...this.questions.map(q => q.displayOrder)) + 1 : 1;
        this.quizService.createQuizQuestion(this.quizId, snapshotJson, newOrder).subscribe({
          next: (created) => {
            const newQuestion: DisplayQuestion = {
              id: created.id,
              content: snapshot.content,
          ...
          error: () => this.message.error(this.translate.instant('quiz.createQuestionError')),
        });
      }
      this.insertIndex = undefined;
    }
  ...
  handleDeleteQuestion(questionId: number): void {
    this.quizService.deleteQuizQuestion(questionId).subscribe({
      ...
      error: () => this.message.error(this.translate.instant('quiz.deleteQuestionError')),
    });
  }
