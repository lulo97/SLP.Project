...
@Component({
  ...
  template: `
    <div class="space-y-4">
      ...
      <!-- List -->
      <nz-spin [nzSpinning]="(quizService.loading$ | async) ?? false" data-testid="quiz-list-loading">
        <nz-list
          nzBordered
          [nzDataSource]="(quizService.quizzes$ | async) ?? []"
          data-testid="quiz-list"
        >
          <nz-list-item *ngFor="let quiz of quizService.quizzes$ | async" [attr.data-testid]="'quiz-list-item-' + quiz.id">
            <div class="flex flex-col w-full">
              <div class="flex justify-between items-start">
                <div class="flex-1">
                  <div class="flex items-center justify-between gap-2">
                    <a [routerLink]="['/quiz', quiz.id]" class="text-base font-medium" [attr.data-testid]="'quiz-title-link-' + quiz.id">
                      {{ quiz.title }}
                    </a>
                    <nz-tag [nzColor]="quiz.visibility === 'public' ? 'green' : 'blue'" [attr.data-testid]="'quiz-visibility-' + quiz.id">
                      {{ quiz.visibility }}
                    </nz-tag>
                  </div>
                  <div class="text-sm text-gray-500 mb-2" [attr.data-testid]="'quiz-description-' + quiz.id">
                    {{ quiz.description || ('quiz.noDescription' | translate) }}
                  </div>
                  <div class="flex items-center justify-between text-xs text-gray-400">
                    <span [attr.data-testid]="'quiz-author-' + quiz.id">
                      {{ 'quiz.by' | translate }} {{ quiz.userName || 'Unknown' }}
                    </span>
                    <span [attr.data-testid]="'quiz-question-count-' + quiz.id">
                      {{ quiz.questionCount }} {{ 'quiz.questions' | translate }}
                    </span>
                  </div>
                  <div class="flex flex-wrap gap-1 mt-1">
                    <nz-tag *ngFor="let tag of quiz.tags" size="small" [attr.data-testid]="'quiz-tag-' + quiz.id + '-' + tag">
                      {{ tag }}
                    </nz-tag>
                  </div>
                </div>
                <div class="flex gap-1">
                  <button nz-button nzType="text" (click)="viewQuiz(quiz.id)" [attr.data-testid]="'view-quiz-' + quiz.id">
                    <i nz-icon nzType="eye"></i>
                  </button>
                  <button nz-button nzType="text" (click)="duplicateQuiz(quiz.id)" [attr.data-testid]="'duplicate-quiz-' + quiz.id">
                    <i nz-icon nzType="copy"></i>
                  </button>
                  <button *ngIf="canEdit(quiz)" nz-button nzType="text" (click)="editQuiz(quiz.id)" [attr.data-testid]="'edit-quiz-' + quiz.id">
                    <i nz-icon nzType="edit"></i>
                  </button>
                  <button *ngIf="canEdit(quiz)" nz-button nzType="text" nzDanger nz-popconfirm [nzPopconfirmTitle]="'common.confirm' | translate" nzOkText="{{ 'common.delete' | translate }}" nzCancelText="{{ 'common.cancel' | translate }}" (nzOnConfirm)="deleteQuiz(quiz.id)" [attr.data-testid]="'delete-quiz-' + quiz.id">
      ...
      <!-- Empty state -->
      <nz-empty
        *ngIf="!(quizService.loading$ | async) && ((quizService.quizzes$ | async) ?? []).length === 0"
        [nzNotFoundContent]="emptyMessage"
        data-testid="quiz-list-empty"
      ></nz-empty>

      <!-- Pagination -->
      <div *ngIf="(quizService.total$ | async) ?? 0 > 0" class="flex justify-center mt-4">
        <nz-pagination
      ...
      <nz-float-button nzType="primary" [nzIcon]="iconTemplate" (click)="goToCreate()" data-testid="create-quiz-fab"></nz-float-button>
...
export class QuizListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.switchTab('my');
    this.searchDebouncer.pipe(debounceTime(500), distinctUntilChanged(), takeUntil(this.destroy$)).subscribe(() => {
    ...
    this.quizService.deleteQuiz(id).subscribe({
      next: () => {
        ...
        if (this.currentTab === 'my') {
          ...
          this.quizService.fetchPublicQuizzes(this.quizService.pageSubject.value);
        }
      },
