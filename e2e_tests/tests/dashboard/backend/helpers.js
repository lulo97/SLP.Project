...
export async function createTestUser(request) {
  ...
  const userData = {
    username: `dash-test-${id}`,
    email: `dash-test-${id}@example.com`,
    password: "Test123!",
  };
...
export async function deleteTestUser(request, adminToken, userId) {
...
export async function loginAsTestUser(request, username, password) {
  const res = await request.post(`${API_BASE_URL}/auth/login`, {
...
export async function deleteQuestion(request, token, questionId) {
  const res = await request.delete(`${API_BASE_URL}/question/${questionId}`, {
    headers: authHeaders(token),
  });
  expect([204, 404]).toContain(res.status());
}
...
export async function deleteSource(request, token, sourceId) {
  const res = await request.delete(`${API_BASE_URL}/source/${sourceId}`, {
    headers: authHeaders(token),
  });
  expect([204, 404]).toContain(res.status());
}
...
export async function deleteFavorite(request, token, favoriteId) {
  const res = await request.delete(`${API_BASE_URL}/favorites/${favoriteId}`, {
    headers: authHeaders(token),
  });
  expect([204, 404]).toContain(res.status());
}
