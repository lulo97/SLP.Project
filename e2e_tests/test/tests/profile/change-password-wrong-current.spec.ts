...
test("change password with wrong current password shows error and does not change password", async ({
  page,
  browser,
}) => {
  ...
  // ---- Step 3: Attempt change password with wrong current password ----
  await test.step("UI: attempt change password (wrong current password)", async () => {
    ...
    await page.getByTestId("input-current-password").fill(wrongCurrentPassword);
    ...
    const errorToast = page.getByText(/Current password is incorrect/i);
  ...
  await test.step("UI: logout and login with original password (should succeed)", async () => {
    ...
    await login(page, userA.username, originalPassword);
    await expect(page).toHaveURL(`${FRONTEND_URL}/dashboard`);
    console.log("Login with original password succeeded – password not changed");
  });
