// e2e_tests/test/tests/comment/edit-multiple-times.spec.ts
...
test.describe("Quiz comment history and deletion flow", () => {
  test("test1 creates quiz, test2 adds and edits comment twice, test1 views history, test1 deletes quiz", async ({
    browser,
  }) => {
    ...
    // Use stable comment ID locator for all subsequent operations
    const commentIdLocator = test2Page.locator(
    ...
    const editInput1 = commentIdLocator.getByTestId("edit-comment-input");
    await expect(editInput1).toBeVisible();
    await editInput1.fill(edit1);
    await commentIdLocator.getByTestId("save-edit-button").click();
    await expect(commentIdLocator).toContainText(edit1);

    // Edit 2
    await commentIdLocator.locator('[data-testid="edit-button"]').click();
    const editInput2 = commentIdLocator.getByTestId("edit-comment-input");
    await expect(editInput2).toBeVisible();
    await editInput2.fill(edit2);
    await commentIdLocator.getByTestId("save-edit-button").click();
    await expect(commentIdLocator).toContainText(edit2);

    // Verify the "edited" indicator appears
    await expect(
      commentIdLocator.locator('[data-testid="comment-edited-indicator"]'),
    ).toBeVisible();
    ...
    // Locate the comment (by its stable ID) and open the history modal
    const comment = test1Page.locator(
      `[data-testid="comment-${numericCommentId}"]`,
    );
