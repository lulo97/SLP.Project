 ...
 * then delete it.
 */
export async function verifyAndDeleteQuestion(page: Page, title: string): Promise<void> {
  ...
  const item = page
    .locator(`li[data-testid^="question-item-"]:has-text("${title}")`)
    .first();
  ...
  if (!questionId) throw new Error(`Could not extract question ID from item with title "${title}"`);
