...
export async function createQuiz(request, token, title, options = {}) {
...
export async function deleteQuiz(request, token, quizId) {
  ...
  if (res.status() !== 204) {
    console.error(`Failed to delete quiz ${quizId}, status: ${res.status()}`);
  }
}
...
export async function createAuthenticatedPage(browser, token) {
  ...
  // Check if redirected (e.g., to dashboard) – if so, throw
  const url = page.url();
