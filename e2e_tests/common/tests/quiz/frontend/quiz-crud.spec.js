...
test.describe("Quiz CRUD", () => {
  ...
  test("Create a quiz with minimal fields", async ({ browser, request }) => {
    ...
    await expect(page.locator('[data-testid="quiz-description"]')).toHaveText(
      `Description for ${title}`,
    );
    await expect(page.locator('[data-testid="quiz-visibility"]')).toHaveText(
      "public",
    );
    ...
    await expect(
      page.locator(`[data-testid="quiz-list-item-${id}"]`),
    ).toBeVisible();
    await expect(
      page.locator(`[data-testid="quiz-title-link-${id}"]`),
    ).toHaveText(title);
  ...
  test("Edit a quiz", async ({ browser, request }) => {
    ...
    await expect(page.locator('[data-testid="quiz-title"]')).toHaveText(
      newTitle,
    );
  ...
  test("Duplicate a quiz", async ({ browser, request }) => {
    ...
    await expect(
      page.locator('.ant-message-success:has-text("Quiz duplicated")'),
    ).toBeVisible();
    await page.waitForURL(/\/quiz\/\d+\/edit$/);
    const duplicatedUrl = page.url();
    const duplicatedId = parseInt(
      duplicatedUrl.split("/").slice(-2, -1)[0],
      10,
    );

    await page.goto(`${FRONTEND_URL}/quiz/${duplicatedId}`);
    // The duplicated quiz gets "(Copy)" appended to its title
    await expect(page.locator('[data-testid="quiz-title"]')).toHaveText(
      `${title} (Copy)`,
    );
  ...
  test("Delete a quiz", async ({ browser, request }) => {
    ...
    await expect(
      page.locator('.ant-message-success:has-text("Quiz deleted")'),
    ).toBeVisible();
