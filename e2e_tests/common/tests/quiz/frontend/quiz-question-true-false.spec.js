...
test.describe("Quiz Question: True/False", () => {
  let authToken;
  ...
  test("Add a true/false question to a quiz", async ({ browser, request }) => {
    const page = await createAuthenticatedPage(browser, authToken);
    const { id } = await createQuiz(page);

    const questionTitle = await addQuestionToQuiz(page, "true_false");
